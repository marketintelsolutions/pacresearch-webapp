...
const TopGainersManager: React.FC = () => {
  const dispatch = useAppDispatch();
  const { gainers, loading, error, success } = useAppSelector((state) => ({
    gainers: state.equityMarket.gainers,
    loading: state.equityMarket.loading.gainers,
  ...
  const initializeEmptyStocks = useCallback(() => {
    return Array.from({ length: 5 }, (_, index) => ({
      id: `gainer_${index}`, // Use consistent IDs
      symbol: "",
      previousDayClose: 0,
      currentDayClose: 0,
      change: 0,
  ...
  useEffect(() => {
    ...
    if (gainers.length > 0) {
      ...
      const paddedGainers = [...gainers];
      while (paddedGainers.length < 5) {
        paddedGainers.push({
          id: `gainer_${paddedGainers.length}`,
          symbol: "",
          previousDayClose: 0,
          currentDayClose: 0,
          change: 0,
          displayOrder: paddedGainers.length,
        });
      }
      setStocks(paddedGainers.slice(0, 5)); // Ensure exactly 5
    } else {
      setStocks(initializeEmptyStocks());
    }
  }, [gainers, initializeEmptyStocks]);
  ...
  const handleSave = async () => {
    ...
    while (stocksToSave.length < 5) {
      stocksToSave.push({
        id: `gainer_empty_${stocksToSave.length}`,
        symbol: "",
        previousDayClose: 0,
        currentDayClose: 0,
        change: 0,
    ...
    try {
      await dispatch(saveTopGainers(stocksToSave.slice(0, 5))).unwrap();
    } catch (err) {
      console.error("Error saving top gainers:", err);
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-xl font-semibold mb-4">Manage Top 5 Gainers</h2>
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-600">
          Enter the top 5 gaining stocks. Drag and drop to reorder. Changes will
        ...
        {stocks.map((stock, index) => (
          ...
          >
            ...
            {dragOverStock === stock.id && dragPosition === "after" && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-green-500 z-10"></div>
            )}
      ...
      <div className="mt-6 flex justify-end">
        ...
        >
          {loading ? "Saving..." : "Save Top Gainers"}
        </button>
      </div>
    </div>
  );
...
export default TopGainersManager;
